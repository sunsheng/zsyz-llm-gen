<template>
  <div class="page-container">
    <PageHeader title="成长榜单" subtitle="企业增长率排名TOP20">
      <template #actions>
    ...
    <div class="chart-grid">
      <div class="chart-panel full-width">
        <h4 class="chart-panel__title">成长率TOP10</h4>
        <BaseChart :option="barOption" height="360px" />
      </div>
    </div>

    <div class="table-section">
      <h4 class="chart-panel__title">成长排名</h4>
      <el-table :data="filteredData" stripe border style="width: 100%">
        <el-table-column prop="rank" label="排名" width="80">
          <template #default="{ row }">
            <span :style="{ fontWeight: row.rank <= 3 ? 700 : 400, color: row.rank <= 3 ? '#4ECB73' : '' }">
              {{ row.rank }}
            </span>
          </template>
        </el-table-column>
        ...
        <el-table-column prop="value" label="营收(万元)" width="140">
          <template #default="{ row }">
...
import { getMockGrowthRanking } from '@/api/mock/monitor'
...
onMounted(() => {
  allData.value = getMockGrowthRanking(20)
  ...
  barOption.value = {
    ...
    yAxis: { type: 'category', data: top10.map((r: any) => r.name).reverse() },
    series: [{
      type: 'bar', barWidth: '50%',
      data: top10.map((r: any) => r.growth).reverse()
    }]
  }
})
