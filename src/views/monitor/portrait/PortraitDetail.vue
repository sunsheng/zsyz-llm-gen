      ...
      <div class="chart-panel">
        ...
        <div class="score-list">
          <div class="score-item">
            ...
            <el-progress :percentage="portrait.healthScore" :color="portrait.healthScore >= 80 ? '#67C23A' : '#1889E8'" :stroke-width="12" />
          </div>
          <div class="score-item">
            <span class="score-item__label">创新评分</span>
            <el-progress :percentage="portrait.innovationScore" :color="portrait.innovationScore >= 80 ? '#67C23A' : '#1889E8'" :stroke-width="12" />
          </div>
          <div class="score-item">
            <span class="score-item__label">成长评分</span>
            <el-progress :percentage="portrait.growthScore" :color="portrait.growthScore >= 80 ? '#67C23A' : '#1889E8'" :stroke-width="12" />
          </div>
          <div class="score-item">
            <span class="score-item__label">风险评分</span>
            <el-progress :percentage="portrait.riskScore" :color="portrait.riskScore <= 30 ? '#67C23A' : '#F56C6C'" :stroke-width="12" />
          </div>
        </div>
      </div>
      <div class="chart-panel">
        ...
        <el-descriptions :column="2" border>
          ...
          <el-descriptions-item label="营收">{{ portrait.revenue.toLocaleString() }}万元</el-descriptions-item>
...
onMounted(() => {
  ...
  radarOption.value = {
    ...
    series: [{
      type: 'radar',
      data: [{
        value: [portrait.value.healthScore, portrait.value.innovationScore, portrait.value.growthScore, portrait.value.riskScore, Math.floor((portrait.value.healthScore + portrait.value.innovationScore + portrait.value.growthScore) / 3)],
  ...
  revenueTrendOption.value = {
    ...
    yAxis: { type: 'value' },
    series: [
      { name: '营收', type: 'line', smooth: true, data: Array.from({ length: 12 }, () => Math.floor(Math.random() * 5000 + 3000)) },
      { name: '利润', type: 'line', smooth: true, data: Array.from({ length: 12 }, () => Math.floor(Math.random() * 1500 + 500)) }
    ]
  }
})
</script>
