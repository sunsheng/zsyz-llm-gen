...
const kpiCards = ref([
  { key: 'ind1', label: '行业总数', value: 6, unit: '个', trend: 'up' as const, trendValue: '+1', icon: 'DataLine', iconColor: '#1889E8', iconBgColor: '#ECF5FF' },
  { key: 'ind2', label: '总产值', value: 358.7, unit: '亿', trend: 'up' as const, trendValue: '+9.8%', icon: 'TrendCharts', iconColor: '#36CBCB', iconBgColor: '#E6F7F7' },
  { key: 'ind3', label: '企业总数', value: 1286, unit: '家', trend: 'up' as const, trendValue: '+15.4%', icon: 'UserFilled', iconColor: '#4ECB73', iconBgColor: '#EDFAF0' },
  { key: 'ind4', label: '平均增长率', value: 8.9, unit: '%', trend: 'up' as const, trendValue: '+2.3%', icon: 'Odometer', iconColor: '#975FE5', iconBgColor: '#F3ECFF' }
])
...
onMounted(() => {
  ...
  barOption.value = {
    ...
    series: [
      ...
      { name: '上年度', type: 'bar', data: contribData.data.series[0].data.map((v: number) => +(v * 0.88).toFixed(1)), barWidth: '25%' }
    ]
  }
