<template>
  <div class="page-container graph-manage-page">
    <PageHeader title="产业链图谱管理" subtitle="凯州新城图谱配置、创建、编辑与企业关联管理">
      <template #actions>
...
const graphs = ref<GraphItem[]>([
  {
    ...
    linkedEnterprises: ['华创科技(凯州)', '凯州智能电气', '盛通精密'],
    layout: 'force',
    nodeSize: 30,
    showLabels: true,
  },
  {
    ...
    linkedEnterprises: ['中科新材料(凯州)', '瑞达材料'],
    layout: 'force',
    nodeSize: 25,
    showLabels: true,
  },
...
async function handleSave() {
  ...
  await formRef.value.validate((valid) => {
    ...
    if (isEditing.value) {
      ...
      const regionObj = regionOptions.find((r) => r.code === formData.region)
      graphs.value.push({
        id: `g${Date.now()}`,
        name: formData.name,
        region: regionObj?.name || '凯州新城',
