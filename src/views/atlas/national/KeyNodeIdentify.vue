<template>
  <div class="page-container key-node-page">
    <PageHeader
      title="产业链关键环节识别"
      subtitle="基于产出、龙头企业、市场份额识别主导环节与薄弱环节"
    >
      <template #actions>
        <IndustryChainSelect v-model="selectedChain" placeholder="选择产业链" />
        <el-select v-model="dimension" placeholder="分析维度" style="width: 160px">
          <el-option label="综合评估" value="composite" />
          <el-option label="产出贡献" value="output" />
          <el-option label="龙头集中度" value="leading" />
          <el-option label="市场份额" value="market" />
        </el-select>
      </template>
    </PageHeader>

    <LoadingSpinner :loading="loading" type="skeleton" :rows="8">
      <!-- 统计卡片 -->
      <div class="key-node-stats">
        <StatCard
          icon="Medal"
          label="主导环节"
          :value="dominantCount"
          unit="个"
          icon-color="#1889E8"
          icon-bg-color="#ECF5FF"
        />
        <StatCard
          icon="Warning"
          label="薄弱环节"
          :value="weakCount"
          unit="个"
          icon-color="#F2637B"
          icon-bg-color="#FEF0F0"
          trend="down"
          trend-value="-3%"
        />
        <StatCard
          icon="TrendCharts"
          label="平均集中度"
          :value="avgConcentration"
          unit="%"
          icon-color="#36CBCB"
          icon-bg-color="#E6FFFB"
        />
        <StatCard
          icon="DataLine"
          label="环节总数"
          :value="totalNodes"
          unit="个"
          icon-color="#975FE5"
          icon-bg-color="#F3F0FF"
        />
      </div>

      <div class="key-node-content">
        <!-- 左侧：关键环节列表 -->
        <div class="node-list-panel">
          <div class="panel-header">
            <h3 class="panel-title">关键环节识别</h3>
            <el-radio-group v-model="filterType" size="small">
              <el-radio-button value="all">全部</el-radio-button>
              <el-radio-button value="dominant">主导</el-radio-button>
              <el-radio-button value="weak">薄弱</el-radio-button>
            </el-radio-group>
          </div>
          <div class="node-list">
            <div
              v-for="node in filteredNodes"
              :key="node.id"
              class="node-card"
              :class="{
                'node-card--dominant': node.type === 'dominant',
                'node-card--weak': node.type === 'weak',
              }"
              @click="handleNodeClick(node)"
            >
              <div class="node-card__header">
                <span class="node-card__name">{{ node.name }}</span>
                <el-tag
                  :type="
                    node.type === 'dominant' ? 'primary' : node.type === 'weak' ? 'danger' : 'info'
                  "
                  size="small"
                >
                  {{ node.type === 'dominant' ? '主导' : node.type === 'weak' ? '薄弱' : '一般' }}
                </el-tag>
              </div>
              <div class="node-card__category">{{ node.category }}</div>
              <div class="node-card__metrics">
                <div class="node-card__metric">
                  <span class="node-card__metric-label">产出贡献</span>
                  <el-progress
                    :percentage="node.outputScore"
                    :stroke-width="6"
                    :color="node.outputScore >= 70 ? '#1889E8' : '#F2637B'"
                    :show-text="false"
                  />
                  <span class="node-card__metric-value">{{ node.outputScore }}%</span>
                </div>
                <div class="node-card__metric">
                  <span class="node-card__metric-label">龙头集中</span>
                  <el-progress
                    :percentage="node.leadingScore"
                    :stroke-width="6"
                    :color="node.leadingScore >= 70 ? '#36CBCB' : '#FBD437'"
                    :show-text="false"
                  />
                  <span class="node-card__metric-value">{{ node.leadingScore }}%</span>
                </div>
                <div class="node-card__metric">
                  <span class="node-card__metric-label">市场份额</span>
                  <el-progress
                    :percentage="node.marketScore"
                    :stroke-width="6"
                    :color="node.marketScore >= 70 ? '#4ECB73' : '#F2637B'"
                    :show-text="false"
                  />
                  <span class="node-card__metric-value">{{ node.marketScore }}%</span>
                </div>
              </div>
              <div class="node-card__enterprises">
                <span class="node-card__enterprises-label">龙头企业：</span>
                <span class="node-card__enterprises-value">{{
                  node.leadingEnterprises.join('、')
                }}</span>
              </div>
            </div>
          </div>
        </div>

        <!-- 右侧：环节详情 -->
        <div class="node-detail-panel">
          <template v-if="activeNode">
            <div class="detail-header">
              <h3 class="detail-title">{{ activeNode.name }}</h3>
              <el-tag
                :type="
                  activeNode.type === 'dominant'
                    ? 'primary'
                    : activeNode.type === 'weak'
                      ? 'danger'
                      : 'info'
                "
              >
                {{
                  activeNode.type === 'dominant'
                    ? '主导环节'
                    : activeNode.type === 'weak'
                      ? '薄弱环节'
                      : '一般环节'
                }}
              </el-tag>
            </div>

            <!-- 评分雷达 -->
            <div class="detail-radar">
              <v-chart :option="radarOption" autoresize style="height: 280px" />
            </div>

            <!-- 环节指标详情 -->
            <div class="detail-metrics">
              <h4 class="detail-section-title">指标详情</h4>
              <el-descriptions :column="2" border size="small">
                <el-descriptions-item label="产出贡献"
                  >{{ activeNode.outputScore }}%</el-descriptions-item
                >
                <el-descriptions-item label="龙头集中度"
                  >{{ activeNode.leadingScore }}%</el-descriptions-item
                >
                <el-descriptions-item label="市场份额"
                  >{{ activeNode.marketScore }}%</el-descriptions-item
                >
                <el-descriptions-item label="综合评分"
                  >{{ activeNode.compositeScore }}分</el-descriptions-item
                >
                <el-descriptions-item label="关联企业数"
                  >{{ activeNode.enterpriseCount }}家</el-descriptions-item
                >
                <el-descriptions-item label="环节层级">{{
                  activeNode.category
                }}</el-descriptions-item>
              </el-descriptions>
            </div>

            <!-- 龙头企业 -->
            <div class="detail-enterprises">
              <h4 class="detail-section-title">龙头企业</h4>
              <el-table :data="activeNode.enterpriseDetails" stripe border>
                <el-table-column prop="name" label="企业名称" min-width="140" />
                <el-table-column prop="marketShare" label="市场份额" width="100">
                  <template #default="{ row }">{{ row.marketShare }}%</template>
                </el-table-column>
                <el-table-column prop="revenue" label="营收(亿)" width="100" align="right" />
              </el-table>
            </div>

            <!-- 补强建议 -->
            <div v-if="activeNode.type === 'weak'" class="detail-suggestion">
              <h4 class="detail-section-title">补强建议</h4>
              <div class="suggestion-list">
                <div v-for="(s, i) in activeNode.suggestions" :key="i" class="suggestion-item">
                  <el-icon :size="16" color="#1889E8"><Promotion /></el-icon>
                  <span>{{ s }}</span>
                </div>
              </div>
            </div>
          </template>
          <el-empty v-else description="请选择环节查看详情" />
        </div>
      </div>
    </LoadingSpinner>
  </div>
</template>

<script setup lang="ts">
import { ref, computed, watch, onMounted } from 'vue'
import { Promotion } from '@element-plus/icons-vue'
import PageHeader from '@/components/common/PageHeader.vue'
import StatCard from '@/components/common/StatCard.vue'
import LoadingSpinner from '@/components/common/LoadingSpinner.vue'
import IndustryChainSelect from '@/components/business/IndustryChainSelect.vue'
import VChart from 'vue-echarts'

interface EnterpriseDetail {
  name: string
  marketShare: number
  revenue: number
}

interface KeyNode {
  id: string
  name: string
  category: string
  type: 'dominant' | 'weak' | 'normal'
  outputScore: number
  leadingScore: number
  marketScore: number
  compositeScore: number
  enterpriseCount: number
  leadingEnterprises: string[]
  enterpriseDetails: EnterpriseDetail[]
  suggestions: string[]
}

const selectedChain = ref<string | string[]>('high-end-equipment')
const dimension = ref('composite')
const filterType = ref('all')
const loading = ref(false)
const activeNode = ref<KeyNode | null>(null)

const nodes = ref<KeyNode[]>([
  {
    id: 'n1',
    name: '整机装配',
    category: '中游',
    type: 'dominant',
    outputScore: 92,
    leadingScore: 88,
    marketScore: 85,
    compositeScore: 88,
    enterpriseCount: 15,
    leadingEnterprises: ['华创科技', '远大智能'],
    enterpriseDetails: [
      { name: '华创科技', marketShare: 28, revenue: 128.5 },
      { name: '远大智能', marketShare: 22, revenue: 215.7 },
      { name: '宏图智造', marketShare: 15, revenue: 189.3 },
    ],
    suggestions: [],
  },
  {
    id: 'n2',
    name: '核心零部件',
    category: '上游',
    type: 'weak',
    outputScore: 35,
    leadingScore: 28,
    marketScore: 22,
    compositeScore: 28,
    enterpriseCount: 3,
    leadingEnterprises: ['盛通精密'],
    enterpriseDetails: [
      { name: '盛通精密', marketShare: 12, revenue: 38.6 },
      { name: '创维光电', marketShare: 8, revenue: 41.8 },
    ],
    suggestions: [
      '引进核心零部件龙头企业补链',
      '加强与中游整机的协同配套',
      '加大研发投入突破关键技术瓶颈',
    ],
  },
  {
    id: 'n3',
    name: '智能控制',
    category: '中游',
    type: 'dominant',
    outputScore: 86,
    leadingScore: 82,
    marketScore: 78,
    compositeScore: 82,
    enterpriseCount: 9,
    leadingEnterprises: ['翔宇半导体', '恒信电子'],
    enterpriseDetails: [
      { name: '翔宇半导体', marketShare: 25, revenue: 276.4 },
      { name: '恒信电子', marketShare: 18, revenue: 45.2 },
    ],
    suggestions: [],
  },
  {
    id: 'n4',
    name: '检测认证',
    category: '中游',
    type: 'weak',
    outputScore: 22,
    leadingScore: 18,
    marketScore: 15,
    compositeScore: 18,
    enterpriseCount: 2,
    leadingEnterprises: ['东方生物'],
    enterpriseDetails: [
      { name: '东方生物', marketShare: 10, revenue: 156.8 },
      { name: '金诺环保', marketShare: 5, revenue: 15.6 },
    ],
    suggestions: [
      '建设区域级检测认证中心',
      '引入国际认证机构提升标准',
      '推动检测认证与产业链深度融合',
    ],
  },
  {
    id: 'n5',
    name: '新能源汽车',
    category: '下游',
    type: 'dominant',
    outputScore: 88,
    leadingScore: 90,
    marketScore: 82,
    compositeScore: 87,
    enterpriseCount: 14,
    leadingEnterprises: ['天成新能源', '宏图智造'],
    enterpriseDetails: [
      { name: '天成新能源', marketShare: 32, revenue: 342.1 },
      { name: '宏图智造', marketShare: 18, revenue: 189.3 },
    ],
    suggestions: [],
  },
  {
    id: 'n6',
    name: '基础材料研发',
    category: '上游',
    type: 'weak',
    outputScore: 38,
    leadingScore: 30,
    marketScore: 25,
    compositeScore: 31,
    enterpriseCount: 3,
    leadingEnterprises: ['中科新材料', '国泰创新'],
    enterpriseDetails: [
      { name: '中科新材料', marketShare: 15, revenue: 86.3 },
      { name: '国泰创新', marketShare: 10, revenue: 63.2 },
    ],
    suggestions: [
      '加强高校科研院所产学研合作',
      '设立专项基金支持基础材料研发',
      '引进国际先进材料企业',
    ],
  },
  {
    id: 'n7',
    name: '航空航天',
    category: '下游',
    type: 'normal',
    outputScore: 65,
    leadingScore: 58,
    marketScore: 55,
    compositeScore: 59,
    enterpriseCount: 11,
    leadingEnterprises: ['华创科技', '鼎盛自动化'],
    enterpriseDetails: [
      { name: '华创科技', marketShare: 20, revenue: 128.5 },
      { name: '鼎盛自动化', marketShare: 12, revenue: 95.7 },
    ],
    suggestions: [],
  },
  {
    id: 'n8',
    name: '特种钢材',
    category: '上游',
    type: 'normal',
    outputScore: 55,
    leadingScore: 48,
    marketScore: 42,
    compositeScore: 48,
    enterpriseCount: 6,
    leadingEnterprises: ['瑞达材料', '国泰创新'],
    enterpriseDetails: [
      { name: '瑞达材料', marketShare: 14, revenue: 28.4 },
      { name: '国泰创新', marketShare: 10, revenue: 63.2 },
    ],
    suggestions: [],
  },
])

const filteredNodes = computed(() => {
  if (filterType.value === 'all') return nodes.value
  return nodes.value.filter((n) => n.type === filterType.value)
})

const dominantCount = computed(() => nodes.value.filter((n) => n.type === 'dominant').length)
const weakCount = computed(() => nodes.value.filter((n) => n.type === 'weak').length)
const avgConcentration = computed(() => {
  if (!nodes.value.length) return 0
  return Math.round(nodes.value.reduce((sum, n) => sum + n.compositeScore, 0) / nodes.value.length)
})
const totalNodes = computed(() => nodes.value.length)

const radarOption = computed(() => {
  if (!activeNode.value) return {}
  return {
    tooltip: {},
    radar: {
      indicator: [
        { name: '产出贡献', max: 100 },
        { name: '龙头集中度', max: 100 },
        { name: '市场份额', max: 100 },
      ],
      shape: 'circle',
      splitNumber: 4,
    },
    series: [
      {
        type: 'radar',
        data: [
          {
            value: [
              activeNode.value.outputScore,
              activeNode.value.leadingScore,
              activeNode.value.marketScore,
            ],
            name: activeNode.value.name,
            areaStyle: { color: 'rgba(24, 137, 232, 0.15)' },
            lineStyle: { color: '#1889E8' },
            itemStyle: { color: '#1889E8' },
          },
        ],
      },
    ],
  }
})

function handleNodeClick(node: KeyNode) {
  activeNode.value = node
}

watch(selectedChain, (val) => {
  loading.value = true
  const code = Array.isArray(val) ? val[val.length - 1] : val
  generateNodesForChain(code)
  setTimeout(() => {
    loading.value = false
  }, 300)
})

function generateNodesForChain(code: string) {
  const chainNodeMap: Record<string, KeyNode[]> = {
    'high-end-equipment': [
      {
        id: 'n1',
        name: '整机装配',
        category: '中游',
        type: 'dominant',
        outputScore: 92,
        leadingScore: 88,
        marketScore: 85,
        compositeScore: 88,
        enterpriseCount: 15,
        leadingEnterprises: ['华创科技', '远大智能'],
        enterpriseDetails: [
          { name: '华创科技', marketShare: 28, revenue: 128.5 },
          { name: '远大智能', marketShare: 22, revenue: 215.7 },
          { name: '宏图智造', marketShare: 15, revenue: 189.3 },
        ],
        suggestions: [],
      },
      {
        id: 'n2',
        name: '核心零部件',
        category: '上游',
        type: 'weak',
        outputScore: 35,
        leadingScore: 28,
        marketScore: 22,
        compositeScore: 28,
        enterpriseCount: 3,
        leadingEnterprises: ['盛通精密'],
        enterpriseDetails: [
          { name: '盛通精密', marketShare: 12, revenue: 38.6 },
          { name: '创维光电', marketShare: 8, revenue: 41.8 },
        ],
        suggestions: [
          '引进核心零部件龙头企业补链',
          '加强与中游整机的协同配套',
          '加大研发投入突破关键技术瓶颈',
        ],
      },
      {
        id: 'n3',
        name: '智能控制',
        category: '中游',
        type: 'dominant',
        outputScore: 86,
        leadingScore: 82,
        marketScore: 78,
        compositeScore: 82,
        enterpriseCount: 9,
        leadingEnterprises: ['翔宇半导体', '恒信电子'],
        enterpriseDetails: [
          { name: '翔宇半导体', marketShare: 25, revenue: 276.4 },
          { name: '恒信电子', marketShare: 18, revenue: 45.2 },
        ],
        suggestions: [],
      },
      {
        id: 'n4',
        name: '检测认证',
        category: '中游',
        type: 'weak',
        outputScore: 22,
        leadingScore: 18,
        marketScore: 15,
        compositeScore: 18,
        enterpriseCount: 2,
        leadingEnterprises: ['东方生物'],
        enterpriseDetails: [
          { name: '东方生物', marketShare: 10, revenue: 156.8 },
          { name: '金诺环保', marketShare: 5, revenue: 15.6 },
        ],
        suggestions: [
          '建设区域级检测认证中心',
          '引入国际认证机构提升标准',
          '推动检测认证与产业链深度融合',
        ],
      },
      {
        id: 'n5',
        name: '新能源汽车',
        category: '下游',
        type: 'dominant',
        outputScore: 88,
        leadingScore: 90,
        marketScore: 82,
        compositeScore: 87,
        enterpriseCount: 14,
        leadingEnterprises: ['天成新能源', '宏图智造'],
        enterpriseDetails: [
          { name: '天成新能源', marketShare: 32, revenue: 342.1 },
          { name: '宏图智造', marketShare: 18, revenue: 189.3 },
        ],
        suggestions: [],
      },
      {
        id: 'n6',
        name: '基础材料研发',
        category: '上游',
        type: 'weak',
        outputScore: 38,
        leadingScore: 30,
        marketScore: 25,
        compositeScore: 31,
        enterpriseCount: 3,
        leadingEnterprises: ['中科新材料', '国泰创新'],
        enterpriseDetails: [
          { name: '中科新材料', marketShare: 15, revenue: 86.3 },
          { name: '国泰创新', marketShare: 10, revenue: 63.2 },
        ],
        suggestions: [
          '加强高校科研院所产学研合作',
          '设立专项基金支持基础材料研发',
          '引进国际先进材料企业',
        ],
      },
    ],
    'frontier-material': [
      {
        id: 'n1',
        name: '材料加工',
        category: '中游',
        type: 'dominant',
        outputScore: 85,
        leadingScore: 80,
        marketScore: 76,
        compositeScore: 80,
        enterpriseCount: 12,
        leadingEnterprises: ['中科新材料', '瑞达材料'],
        enterpriseDetails: [
          { name: '中科新材料', marketShare: 22, revenue: 86.3 },
          { name: '瑞达材料', marketShare: 15, revenue: 28.4 },
        ],
        suggestions: [],
      },
      {
        id: 'n2',
        name: '矿产开采',
        category: '上游',
        type: 'normal',
        outputScore: 58,
        leadingScore: 50,
        marketScore: 48,
        compositeScore: 52,
        enterpriseCount: 8,
        leadingEnterprises: ['凯州矿业'],
        enterpriseDetails: [{ name: '凯州矿业', marketShare: 18, revenue: 45.2 }],
        suggestions: [],
      },
      {
        id: 'n3',
        name: '合成制备',
        category: '中游',
        type: 'weak',
        outputScore: 32,
        leadingScore: 25,
        marketScore: 20,
        compositeScore: 26,
        enterpriseCount: 2,
        leadingEnterprises: ['博远化工'],
        enterpriseDetails: [{ name: '博远化工', marketShare: 8, revenue: 22.1 }],
        suggestions: ['加大合成制备技术引进', '与高校合作建设重点实验室'],
      },
      {
        id: 'n4',
        name: '性能改性',
        category: '中游',
        type: 'weak',
        outputScore: 28,
        leadingScore: 22,
        marketScore: 18,
        compositeScore: 23,
        enterpriseCount: 2,
        leadingEnterprises: ['恒信材料'],
        enterpriseDetails: [{ name: '恒信材料', marketShare: 6, revenue: 15.3 }],
        suggestions: ['引进改性技术领军企业', '加强产学研合作'],
      },
    ],
    'digital-economy': [
      {
        id: 'n1',
        name: '软件开发',
        category: '中游',
        type: 'dominant',
        outputScore: 90,
        leadingScore: 85,
        marketScore: 82,
        compositeScore: 86,
        enterpriseCount: 18,
        leadingEnterprises: ['博远信息', '翔宇半导体'],
        enterpriseDetails: [
          { name: '博远信息', marketShare: 20, revenue: 67.9 },
          { name: '翔宇半导体', marketShare: 25, revenue: 276.4 },
        ],
        suggestions: [],
      },
      {
        id: 'n2',
        name: '晶圆制造',
        category: '上游',
        type: 'weak',
        outputScore: 25,
        leadingScore: 18,
        marketScore: 15,
        compositeScore: 19,
        enterpriseCount: 1,
        leadingEnterprises: ['翔宇半导体'],
        enterpriseDetails: [{ name: '翔宇半导体', marketShare: 30, revenue: 276.4 }],
        suggestions: ['引进晶圆代工龙头企业', '建设晶圆制造产业园区'],
      },
      {
        id: 'n3',
        name: '模组组装',
        category: '中游',
        type: 'dominant',
        outputScore: 82,
        leadingScore: 78,
        marketScore: 75,
        compositeScore: 78,
        enterpriseCount: 11,
        leadingEnterprises: ['恒信电子', '创维光电'],
        enterpriseDetails: [
          { name: '恒信电子', marketShare: 18, revenue: 45.2 },
          { name: '创维光电', marketShare: 12, revenue: 41.8 },
        ],
        suggestions: [],
      },
      {
        id: 'n4',
        name: '算法优化',
        category: '中游',
        type: 'weak',
        outputScore: 30,
        leadingScore: 22,
        marketScore: 20,
        compositeScore: 24,
        enterpriseCount: 2,
        leadingEnterprises: ['博远信息'],
        enterpriseDetails: [{ name: '博远信息', marketShare: 10, revenue: 67.9 }],
        suggestions: ['引进AI算法团队', '建设算法创新平台'],
      },
    ],
  }
  const defaultNodes: KeyNode[] = [
    {
      id: 'n1',
      name: '生产制造',
      category: '中游',
      type: 'dominant',
      outputScore: 78,
      leadingScore: 72,
      marketScore: 68,
      compositeScore: 73,
      enterpriseCount: 10,
      leadingEnterprises: ['华创科技'],
      enterpriseDetails: [{ name: '华创科技', marketShare: 20, revenue: 128.5 }],
      suggestions: [],
    },
    {
      id: 'n2',
      name: '基础研发',
      category: '上游',
      type: 'weak',
      outputScore: 30,
      leadingScore: 22,
      marketScore: 18,
      compositeScore: 23,
      enterpriseCount: 2,
      leadingEnterprises: ['中科新材料'],
      enterpriseDetails: [{ name: '中科新材料', marketShare: 12, revenue: 86.3 }],
      suggestions: ['加大研发投入', '引进高端人才'],
    },
  ]
  nodes.value = chainNodeMap[code] || defaultNodes
  activeNode.value = null
}

onMounted(() => {
  loading.value = true
  setTimeout(() => {
    loading.value = false
  }, 300)
})
</script>

<style lang="scss" scoped>
.key-node-stats {
  display: grid;
  grid-template-columns: repeat(4, 1fr);
  gap: 20px;
  margin-bottom: 20px;
}

.key-node-content {
  display: flex;
  gap: 20px;
  height: calc(100vh - #{$header-height} - 240px);
}

.node-list-panel {
  flex-shrink: 0;
  width: 380px;
  overflow-y: auto;
  background: $bg-card;
  border-radius: $radius-base;
  box-shadow: $shadow-card;
}

.panel-header {
  display: flex;
  align-items: center;
  justify-content: space-between;
  padding: 16px;
  border-bottom: 1px solid $border-color-lighter;
}

.panel-title {
  margin: 0;
  font-size: 15px;
  font-weight: $font-weight-bold;
  color: $text-primary;
}

.node-list {
  padding: 12px;
}

.node-card {
  padding: 16px;
  margin-bottom: 12px;
  cursor: pointer;
  border: 1px solid $border-color-lighter;
  border-radius: $radius-base;
  transition:
    border-color $transition-fast,
    box-shadow $transition-fast;

  &:hover {
    border-color: $color-primary;
    box-shadow: 0 2px 8px 0 rgba(24, 137, 232, 0.1);
  }

  &--dominant {
    border-left: 3px solid #1889e8;
  }

  &--weak {
    border-left: 3px solid #f2637b;
  }
}

.node-card__header {
  display: flex;
  align-items: center;
  justify-content: space-between;
  margin-bottom: 4px;
}

.node-card__name {
  font-size: 14px;
  font-weight: $font-weight-medium;
  color: $text-primary;
}

.node-card__category {
  margin-bottom: 12px;
  font-size: 12px;
  color: $text-secondary;
}

.node-card__metrics {
  display: flex;
  flex-direction: column;
  gap: 8px;
  margin-bottom: 12px;
}

.node-card__metric {
  display: flex;
  gap: 8px;
  align-items: center;
}

.node-card__metric-label {
  flex-shrink: 0;
  width: 56px;
  font-size: 12px;
  color: $text-secondary;
}

.node-card__metric .el-progress {
  flex: 1;
}

.node-card__metric-value {
  flex-shrink: 0;
  width: 32px;
  font-size: 12px;
  color: $text-regular;
  text-align: right;
}

.node-card__enterprises {
  font-size: 12px;
  color: $text-secondary;
}

.node-card__enterprises-label {
  color: $text-placeholder;
}

.node-card__enterprises-value {
  color: $text-regular;
}

.node-detail-panel {
  flex: 1;
  min-width: 0;
  padding: 20px;
  overflow-y: auto;
  background: $bg-card;
  border-radius: $radius-base;
  box-shadow: $shadow-card;
}

.detail-header {
  display: flex;
  gap: 12px;
  align-items: center;
  margin-bottom: 20px;
}

.detail-title {
  margin: 0;
  font-size: 18px;
  font-weight: $font-weight-bold;
  color: $text-primary;
}

.detail-section-title {
  margin: 20px 0 12px;
  font-size: 14px;
  font-weight: $font-weight-medium;
  color: $text-primary;
}

.detail-radar {
  margin-bottom: 20px;
}

.suggestion-list {
  display: flex;
  flex-direction: column;
  gap: 10px;
}

.suggestion-item {
  display: flex;
  gap: 8px;
  align-items: flex-start;
  font-size: 13px;
  color: $text-regular;
}
</style>
