      ...
      <el-table :data="strategyData" stripe border style="width: 100%">
        ...
        <el-table-column prop="type" label="组合类型" width="120">
          <template #default="{ row }">
            <el-tag :type="(row.type === 'SO' ? 'success' : row.type === 'WO' ? 'warning' : row.type === 'ST' ? '' : 'danger') as any" size="small">{{ row.type }}</el-tag>
          </template>
        </el-table-column>
...
const filterFields = [
  { key: 'industry', label: '产业', type: 'select' as const, options: [
...
const strategyData = ref([
  { strategy: 'SO战略', type: 'SO', description: '发挥区位和数字产业优势，抢抓长三角一体化机遇，加快数字经济和高端装备制造招商', priority: '高' },
  { strategy: 'SO战略', type: 'SO', description: '利用人才优势，承接新兴技术产业转移，打造创新高地', priority: '高' },
  { strategy: 'WO战略', type: 'WO', description: '通过城市更新和低效用地再开发缓解土地约束，引进高附加值产业', priority: '中' },
  { strategy: 'WO战略', type: 'WO', description: '借助政策机遇推动传统制造业数字化转型，降低转型成本', priority: '中' },
  { strategy: 'ST战略', type: 'ST', description: '强化差异化招商策略，聚焦细分领域打造特色产业集群', priority: '高' },
  { strategy: 'ST战略', type: 'ST', description: '构建产业技术联盟，降低技术迭代带来的风险', priority: '中' },
  { strategy: 'WT战略', type: 'WT', description: '优化中小企业扶持政策，缓解融资难题，防范产业空心化', priority: '中' },
  { strategy: 'WT战略', type: 'WT', description: '加强环保技术与产业融合，将环保约束转化为绿色产业机遇', priority: '低' }
])
