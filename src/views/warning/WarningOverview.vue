      ...
      <el-table :data="alertData" stripe border style="width: 100%">
        ...
        <el-table-column prop="type" label="风险类型" width="120">
          <template #default="{ row }">
            <el-tag :type="(row.type === '企业风险' ? 'danger' : row.type === '行业风险' ? 'warning' : row.type === '政策风险' ? 'info' : 'info') as any" size="small">{{ row.type }}</el-tag>
          </template>
        </el-table-column>
...
const kpiCards = [
  { key: 'total', label: '风险预警', value: 186, unit: '条', trend: 'down' as const, trendValue: '-5.2%', icon: 'Warning', iconColor: '#F2637B', iconBgColor: '#FEF0F0' },
  { key: 'high', label: '高风险', value: 28, unit: '条', trend: 'up' as const, trendValue: '+2', icon: 'CircleClose', iconColor: '#F56C6C', iconBgColor: '#FEF0F0' },
  { key: 'medium', label: '中风险', value: 86, unit: '条', trend: 'down' as const, trendValue: '-8', icon: 'Warning', iconColor: '#E6A23C', iconBgColor: '#FDF6EC' },
  { key: 'low', label: '低风险', value: 72, unit: '条', trend: 'down' as const, trendValue: '-3', icon: 'CircleCheck', iconColor: '#67C23A', iconBgColor: '#EDFAF0' }
]
...
onMounted(() => {
  const names = ['华创新材料科技', '东方精密制造', '博远生物医药', '天域半导体', '绿能新能源', '中科智联信息', '鼎盛环保科技', '创想数字技术']
  ...
  alertData.value = names.slice(0, 8).map((name, i) => ({
    ...
    desc: ['经营现金流持续下降', '行业产能过剩风险加剧', '环保政策趋严影响生产', '核心供应商断供风险'][i % 4]
  }))
