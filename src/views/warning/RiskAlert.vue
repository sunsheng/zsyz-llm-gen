<template>
  <div class="page-container">
    ...
    <div class="alert-list">
      <div v-for="alert in filteredAlerts" :key="alert.id" class="alert-item" :class="{ 'alert-item--unread': !alert.read }">
        <div class="alert-item__header">
          <el-tag :type="alert.level === 'critical' ? 'danger' : alert.level === 'high' ? 'danger' : alert.level === 'medium' ? 'warning' : 'success'" size="small">
            {{ alert.level === 'critical' ? '极高风险' : alert.level === 'high' ? '高风险' : alert.level === 'medium' ? '中风险' : '低风险' }}
          </el-tag>
...
const kpiCards = [
  { key: 'total', label: '今日预警', value: 18, unit: '条', trend: 'up' as const, trendValue: '+3', icon: 'Bell', iconColor: '#F2637B', iconBgColor: '#FEF0F0' },
  { key: 'unread', label: '未读预警', value: 6, unit: '条', trend: 'up' as const, trendValue: '+2', icon: 'Message', iconColor: '#1889E8', iconBgColor: '#ECF5FF' },
  { key: 'critical', label: '紧急预警', value: 3, unit: '条', trend: 'flat' as const, trendValue: '', icon: 'Warning', iconColor: '#F56C6C', iconBgColor: '#FEF0F0' },
  { key: 'handled', label: '已处置', value: 12, unit: '条', trend: 'up' as const, trendValue: '+5', icon: 'CircleCheck', iconColor: '#67C23A', iconBgColor: '#EDFAF0' }
]
...
const filteredAlerts = computed(() => {
  ...
  if (keyword.value) {
    data = data.filter((d: any) => d.enterprise.includes(keyword.value) || d.title.includes(keyword.value))
  }
  return data
})
...
onMounted(() => {
  ...
  const levels: Array<'low' | 'medium' | 'high' | 'critical'> = ['critical', 'high', 'medium', 'low']
  const types = ['企业风险', '行业风险', '政策风险', '供应链风险']
  const enterprises = ['华创新材料科技', '高端装备制造行业', '新材料产业', '天域半导体', '东方精密制造', '绿能新能源', '博远生物医药', '电子信息行业', '鼎盛环保科技', '创想数字技术', '原材料市场', '恒达装备制造']
  const descs = [
