  ...
  <div class="page-container">
    ...
    <div class="select-section">
      <div class="chart-panel">
        ...
        <div class="select-row">
          <el-select v-model="selectedRegions" multiple placeholder="请选择区域（最多4个）" :max-collapse-tags="3" style="width: 500px">
    ...
    <div class="table-section">
      ...
      <el-table :data="compareTableData" stripe border style="width: 100%">
        ...
        <el-table-column v-for="(region, idx) in compareRegions" :key="idx" :label="region.name" min-width="120">
...
function handleCompare() {
  const regionNames: Record<string, string> = { hz: '杭州市', nb: '宁波市', wz: '温州市', jx: '嘉兴市', hz2: '湖州市', sx: '绍兴市', jh: '金华市', tz: '台州市' }
  ...
  compareTableData.value = indicators.map((indicator, i) => ({
    indicator,
    unit: '',
    values: compareRegions.value.map(r => i === 0 ? r.gdp : i === 1 ? r.growth : i === 2 ? r.enterprises : i === 3 ? r.investAmount : Math.floor(Math.random() * 1000 + 100))
  }))
  ...
  gdpBarOption.value = {
    ...
    series: [
      { name: 'GDP(亿元)', type: 'bar', barWidth: '25%', data: compareRegions.value.map(r => r.gdp) },
      { name: '增速(%)', type: 'line', yAxisIndex: 1, smooth: true, data: compareRegions.value.map(r => r.growth) }
    ]
  }
}
</script>
